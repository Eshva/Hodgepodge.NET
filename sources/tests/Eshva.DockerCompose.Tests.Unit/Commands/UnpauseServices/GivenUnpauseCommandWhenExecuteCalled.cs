...
namespace Eshva.DockerCompose.Tests.Unit.Commands.UnpauseServices
{
    public sealed class GivenUnpauseCommandWhenExecuteCalled
    {
        [Fact]
        public async Task ShouldContainValidArguments()
        {
            ...
            Expression<Func<string, bool>> argumentsValidator =
                arguments => arguments.Equals("-f \"project.yaml\" unpause", StringComparison.OrdinalIgnoreCase);
            starterMock.Setup(starter => starter.Start(It.Is(argumentsValidator), It.IsAny<TimeSpan>()))
                       .Returns(Task.FromResult(0));
            var command = UnpauseServicesCommand.WithFilesAndStarter(starterMock.Object, "project.yaml")
                                                .AllServices()
                                                .Build();
